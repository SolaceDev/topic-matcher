...
public class MinimalBeerTopicGenerator {
    private final Logger log = LoggerFactory.getLogger(MinimalBeerTopicGenerator.class);
    ...
    public MinimalBeerTopicGenerator(double chanceOfStar, double changeOfGT) {
    ...
    public String deriveSubscription(String topic) {
        List<String> subScriptionParts = new ArrayList<>();
        ...
        return String.join("/", subScriptionParts);
    }

    public List<String> generatePublisherTopics() {
        List<String> ret = new LinkedList<>();

        for (String domain : orderDomains) {
                for (String action : actions) {
                    ret.add(String.format("%s/{prodType}/%s/{version}/{area}/{deliveryAction}/{custId}/{prodId" +
                                    "}/{ordId}", domain,
                            action));
                }
        }
